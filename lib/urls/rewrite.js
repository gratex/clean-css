...
function rebase(uri, options, context) {
  var cacheBust = context.options && context.options.cacheBust;
  if(cacheBust && !isSVGMarker(uri) && !isData(uri) && !isImport(uri) && !isInternal(uri)) {
    // add cache bust
    var cacheBust
    if(/cacheBust=[0-9]+/.test(uri)) {
      uri = uri.replace(/cacheBust=[0-9]+/,`cacheBust=${cacheBust}`);
    } else if (/\?/.test(uri)) {
      uri += `&cacheBust=${cacheBust}`;
    } else {
      uri += `?cacheBust=${cacheBust}`;
    }    
  }
...
function rewriteUrls(data, options, context) {
  return reduceUrls(data, context, function (originUrl, tempData) {
    ...
    tempData.push('url(' + quote + rebase(url, options, context) + quote + ')');
  });
}
